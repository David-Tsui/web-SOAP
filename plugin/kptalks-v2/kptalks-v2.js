(function($) {
  ...
  $.kptalks = function(options) {
    ...
    var settings = $.extend({
        ...
        comein_position: 60, // show kp after scroll more than percent of page height
        default_text: '123test~', // the words show in popup before loading done
        enter_from: 'left', // options: left, right
        enter_distance: -30 // the distance to window side
    }, options);
      ...
      if((scroll+window_h) > (page_h*(settings.comein_position/100))) {
        if(container.css(settings.enter_from) == '-'+settings.width+'px') {
          switch(settings.effect) {
            ...
            case 'slow':
              container.animate(come_in, 500, function() {
                popupIn(settings.popup_effect);
              });
              break;
            ...
            case 'sneaky':
              ...
              container
                .animate(sneaky_pos1, 50).delay(100)
                .animate(sneaky_pos2, 50).delay(200)
                .animate(sneaky_pos1, 50).delay(200)
                .animate(sneaky_pos3, 50).delay(100)
                .animate(come_in, 500, function() {
                  popupIn(settings.popup_effect);
                });
              break;
  ...
  function createKP(settings){
    ...
    if(settings.enter_from == 'left')
      ...
      arrow_pos = 'left';
    var object = '<div id="kp_come_container" style="width:' + settings.width + 'px; height:' + settings.height + 'px; ' + settings.enter_from + ':-' + settings.width + 'px; bottom:0;">' +
                 '<img src="' + img_src + '" style="width:' + settings.width + 'px; height:' + settings.height + 'px;">' +
                 '<div id="kp_popup" style="' + settings.enter_from + ':' + ((settings.width)*0.8) + 'px;top:' + (settings.height*0.28) + 'px;-webkit-border-radius:' + settings.popup_radius + ';-moz-border-radius:' + settings.popup_radius + ';border-radius:' + settings.popup_radius + ';background-color:' + settings.popup_bgcolor + '">' +
                 '<div id="kp_says" style="color:' + settings.popup_color + '">' + settings.default_text + '<a href="http://kptaipei.tw" target="_blank" class="kp_readmore" style="color:'  +  settings.readmore_color  +  '">了解更多柯文哲的政見</a></div>' +
                 '<div id="kp_popup_arrow_shadow" style="border-' + arrow_pos + ': 40px solid rgba(0,0,0,.1);' + settings.enter_from + ': -40px;"></div>' +
                 '<div id="kp_popup_arrow" style="border-' + arrow_pos + ': 42px solid ' + settings.popup_bgcolor + ';' + settings.enter_from + ': -40px;"></div>' +
                 '<div id="kp_close_popup">X</div></div></div>';
    $('body').append(object);
    //loadData(settings);
  }
